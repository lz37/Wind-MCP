...
def wind_call_with_resilience(func: Callable[P, R], *args: P.args, **kwargs: P.kwargs) -> R:
    ...
    The callback runs OUTSIDE the session lock: serialization is guaranteed by
    the single-thread executor, and holding the lock across the call would let
    one wedged SDK call block reconnect on the same lock forever (observed: a
    stuck WSD held the lock 30s+ and froze recovery).  The lock only guards
    lifecycle state (get / invalidate / reconnect).

    The raw WindData response is returned unchanged so the existing parser
    remains responsible for raising ``WindAPIError``.
    """
